...
class EnhancedNPMCompromiseDetectorPhoenix:
    def __init__(self, config_file: str = None, phoenix_config_file: str = None):
        ...
        self.phoenix_config_file = phoenix_config_file or ".phoenix.config"
    ...
    def load_phoenix_config(self) -> Dict:
        """Load Phoenix API configuration from embedded credentials, environment variables, or .phoenix.config file"""
        config = {}
    ...
    def create_config_template(self, output_path: str = ".phoenix.config.template"):
...
assessment_name = NPM Compromise Detection - SANDWORM_MODE
import_type = new
...
"""
        ...
        print(f"💡 Copy this to .phoenix.config and fill in your credentials")
    ...
    def create_phoenix_finding(self, package_name: str, version: str, severity: str,
        ...
        base_risk_mapping = {
            ...
            'CLEAN': 50             # Clean library not affected by SANDWORM_MODE
        }
        ...
        phoenix_severity_mapping = {
            ...
            'CLEAN': 1.0            # Clean library not affected by SANDWORM_MODE
        }
        ...
        finding = {
            ...
            "tags": [
                {"value": "SANDWORM_MODE"},
                {"value": "supplychain"},
                {"value": "sandworm-compromised-package" if not is_safe and severity != 'CLEAN' else "sandworm-clean-library"},
                {"value": "npm-security"},
                {"value": "compromise-detection"}
            ...
            "details": {
                ...
                "scan_tool": "SANDWORM_MODE NPM Compromise Detector v2.0",
    ...
    def _save_debug_payload(self, payload: Dict[str, Any]):
        ...
        try:
            ...
            for asset in payload.get('assets', []) or []:
                asset_attrs = asset.get('attributes', {}) or {}
                asset_repo = asset_attrs.get('repository')
                asset_build_file = asset_attrs.get('buildFile')
                for finding in asset.get('findings', []) or []:
                    details = finding.get('details', {}) or {}
                    packages = finding.get('packages') or []
                    if packages:
                        for pkg in packages:
                            packages_sent.append({
                                "package": pkg.get("name") or details.get("package_name"),
                                "version": pkg.get("version") or details.get("package_version"),
                                "severity": finding.get("severity"),
                                "risk_score": details.get("risk_score"),
                                "risk_level": details.get("risk_level"),
                                "repository": asset_repo,
                                "build_file": asset_build_file,
                        ...
                        packages_sent.append({
                            ...
                            "version": details.get("package_version"),
                            "severity": finding.get("severity"),
                            "risk_score": details.get("risk_score"),
                            "risk_level": details.get("risk_level"),
                            "repository": asset_repo,
            ...
            packages_file = os.path.join(debug_dir, f"phoenix_packages_{timestamp}.json")
            with open(packages_file, 'w', encoding='utf-8') as f:
                json.dump(packages_sent, f, indent=2, ensure_ascii=False)

            print(f"🐛 Debug: Payload saved to {payload_file}")
            print(f"🐛 Debug: Summary saved to {summary_file}")
            print(f"🐛 Debug: Packages saved to {packages_file}")
    ...
    def _load_default_data(self):
        ...
        self.compromised_packages = {
            "claud-code": {"compromised_versions": ["0.2.1"], "safe_version": None},
            "cloude-code": {"compromised_versions": ["0.2.1"], "safe_version": None},
            "cloude": {"compromised_versions": ["0.3.0"], "safe_version": None},
            "crypto-locale": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "crypto-reader-info": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "detect-cache": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "format-defaults": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "hardhta": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "locale-loader-pro": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "naniod": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "node-native-bridge": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "opencraw": {"compromised_versions": ["2026.2.17"], "safe_version": None},
            "parse-compat": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "rimarf": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "scan-store": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "secp256": {"compromised_versions": ["1.0.0"], "safe_version": None},
            "suport-color": {"compromised_versions": ["1.0.1"], "safe_version": None},
            "veim": {"compromised_versions": ["2.46.2"], "safe_version": None},
            "yarsg": {"compromised_versions": ["18.0.1"], "safe_version": None},
        }
        ...
        self.crypto_indicators = [
            "AES-256-GCM",
            "stage2_sha256:5440e1a424631192dff1162eebc8af5dc2389e3d3b23bd26e9c012279ae116e4"
        ]
        ...
        if self.import_all_libraries:
            for lib in self.clean_libraries:
                if lib.get('file') == file_path:  # Only process libraries from this specific file
                    ...
                    report_finding = {
                        'severity': 'CLEAN',
                        'message': f"Library {lib['name']} version {lib['clean_version']} is not affected by SANDWORM_MODE",
                        'file': asset_file_path,  # Use repository path for consistent reporting
                        'repo_url': repo_url,
                        'details': {
                            'package': lib['name'],
    ...
    def get_github_api_headers(self, use_auth: bool = True) -> Dict[str, str]:
        ...
        headers = {
            ...
            'User-Agent': 'SANDWORM_MODE-NPM-Security-Scanner/1.0'
        }
    ...
    def fetch_all_user_repositories(self) -> List[str]:
        ...
        if not self.github_token or self.github_token == 'your_github_token_here':
            ...
            print("💡 Set GITHUB_TOKEN environment variable or configure in .phoenix.config file")
            return repositories
...
def main():
    ...
    parser.add_argument('--repo-url', type=str,
                       help='Repository URL for the target or a single remote repo (with --light-scan)')
    ...
    parser.add_argument('--phoenix-config', default='.phoenix.config',
    ...
    target_is_repo_url = isinstance(args.target, str) and (
        args.target.startswith('http://') or
        args.target.startswith('https://') or
        args.target.startswith('git@github.com:')
    )

    handled_remote = False
    if args.light_scan and not args.pull_all and not args.repo_list and not args.folder_list and not args.folders:
        repo_url = args.repo_url or (args.target if target_is_repo_url else None)
        if repo_url:
            assets = detector.light_scan_repository(repo_url)
            detector.phoenix_assets = assets
            handled_remote = True

    if not handled_remote and args.pull_all:
